...
pf_prep <- function(stats, 
  ...
  stats <- stats[, .(region, tank_id, 
                     ...
                     'BATTLES.LIM' = sum(get(measure.battles))), by=account_id]
  ...
  if (!is.null(max.battles)) {
    ...
    if (measure.relative) {
      stats[measure.battles > max.battles, 
  ...
  tank_ids <- as.integer(colnames(res.DT))
  ...
  return(list('account_ids'=account_ids, 
              'tank_ids'=tank_ids, 
...
pf_est_factors <- function(btl_mtrx=NULL, res_mtrx=NULL, 
  ...
  modes <- list("default"=list("rounds"=100,  "MSE"=1e-14), 
  ...
  # starting values
  ## players' average WR
  if (is.null(player.skill)) { player.skill <- rowSums(res_mtrx) / rowSums(btl_mtrx) }
  ## tanks' average WR  
  if (is.null(tank.perf)) { tank.perf <- 2*colSums(res_mtrx) / colSums(btl_mtrx) }
  ...
  for (i in seq_len(rounds)) {
    ...
    if (abs(MSE - MSE.old)/MSE <= MSE.delta) { break }
    MSE.old <- MSE
...
pf_est_gen_res <- function(player.skill, tank.perf, MSE, R2) {
  return(list('player.skill' = player.skill, 
              'tank.perf' = tank.perf, 
              'MSE'=MSE, 
              'R-squared' = R2))
}
...
pf_get_player_skill  <- function(params, perf, topN=0, min.battles=NULL, only_region=NULL) {
  ...
  dat <- data.table('account_id'=account_ids, 
                    'region'=regions, 
