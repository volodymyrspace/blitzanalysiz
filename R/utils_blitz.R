...
get_stats_tank <- function(tank_ID, DT = NULL) {
  ...
  if (is.null(DT)) {
    found = FALSE
    statDTs <- c('stats.update.tank.perf', 'stats.update.tank', 
...
calc_tank_stats <- function(DT, career_stats = FALSE) {
  ...
  DT[, c('WR.tier.all',
        'avg_dmg.tier.all',
        'spot_rate.tier.all') := list(sum(all.wins)/first(battles.tier.all),
  ...
  DT[ (! is.na(battles.tier.maxed)) &
       DT_filter_battles_tier(battles.tier.maxed, tier),
     c('WR.tier.maxed',
       'avg_dmg.tier.maxed',
       'spot_rate.tier.maxed',
       'tanks.tier.maxed') := list(sum(all.wins) / first(battles.tier.maxed),
  ...
  DT[  (! is.na(tanks.tier.maxed)) & (tanks.tier.maxed > 1) &
...
drop_cols.clean <- function(DT) {
  cols.drop <- c('max_xp', 'in_garage_updated', 'max_frags',
...
drop_cols.slim <- function(DT) {
  ...
  return(drop_cols(DT, c(drop.cols.diff,
...
calc_diff_tank_stats <- function(stats.start, stats.period, career = FALSE, drop.old = TRUE) {
  ...
  account_ids.old_tanks <- as.character(stats.diff[ (battles.old > 0) &
                            (battles.new > battles.old),
                            unique(account_id) ])
  account_ids.new_tanks <- as.character(stats.diff[ battles.old == 0,
                            unique(account_id) ])
  ...
  for (col in c("battles","all.capture_points", "all.damage_dealt", "all.damage_received",
...
get_tank_list <- function(stats = NULL, tank_nation = NA, tank_type = NA, tank_tier = NA, premium = NA) {
  if (is.null(stats)) {
    ...
    tanks2list <- unique(stats[ (is.na(tank_nation) | (nation == tank_nation)) &
                                  ( is.na(tank_type) | (type == tank_type)) &
                                  ( is.na(tank_tier) | (tier == tank_tier)) &
                                  ( is.na(premium) | (is_premium == premium)),
                                .(tier = tier, nation = nation, type = type, is_premium = is_premium, name = name),
                                by=tank_id])[order(tier, nation, type, name)]
...
filename_period <- function(update.start, update.end) {
  return(filename_data(paste0("tank_stats_period_", update.start, '-',
                update.end, ".qs")));
}


filename_period_totals <- function(update.start, update.end) {
  return(filename_data(paste0("tank_stats_period_totals_", update.start, '-',
                update.end, ".qs")));
}
...
get_next_update <- function(update, preliminary = FALSE) {
  ...
  if ((updates[row+1, `Cut-off`] < 1e17) || ( preliminary && updates[row, `Cut-off`] < 1e17)) {
...
get_latest_update <- function(preliminary = FALSE) {
  stopifnot(exists('updates'));
  latest_row = tail(updates[`Cut-off` < 1e17, which = TRUE], n=1)
  if (preliminary) {
...
is_preliminary <- function(update = 'None') {
  ...
  if (updates[ ,update %in% Release ] &&
      (updates[ Release == update, `Cut-off`] > 1e17)) {
    return(TRUE)
  }
  return(FALSE)
}
...
get_tankopedia <- function() {
  ...
  colnames(tanks) <- c('tank_id', 'name', 'nation', 'type', 'tier', 'is_premium', 'next_tanks')
...
gen_tank_stats_career <- function(update, force=FALSE) {
  ...
  stats.after <- run_parts(stats.before, func=function(DT, DT.update) { 
  ...
  while (! is.null(next_update)) {
    ...
    if ((force) || ( ! stats.exists(filename_stats.update)) ) {
      ...
      for (i in seq(1, length(stats.cum))) {
        stats.update_totals[[i]] <- subset(stats.update_totals[[i]],
                         account_id %in% account_ids.curr)
        stats.cum[[i]]           <- subset(stats.cum[[i]],
...
gen_tank_stats_period <- function(update.start, update.end, force = FALSE, verbose = TRUE) {
  ...
  stats.period[[i]] <- calc_diff_tank_stats(stats.cum[[1]],
                        stats.period_totals[[1]], career = TRUE)
  stats.cum[[1]]           <- NULL
...
# gen_update_tank_stats.depreciated <- function(base_update=NULL, target_update = get_latest_update(),
# 																	force = FALSE, save_all = FALSE, save_cum = FALSE, preliminary = FALSE) {
# 	# generate update (delta) stats
# 	stopifnot(exists('tanks'), exists('updates'), exists('WG_tank_stats'))
#
# 	if (is.null(base_update)) {
# 		base_update <- get_prev_update(target_update)
# 	}
# 	filename_stats.cum <- filename_cumulative(base_update)
# 	stopifnot(file.exists(filename_stats.cum))
#
# 	curr_update <- base_update
# 	next_update <- get_next_update(curr_update, preliminary)
#
...
# 		if ( ! exists('stats.cum')) {
# 			print(paste('Update', curr_update, ': Reading cumulative stats'))
# 			stats.cum <- qread(filename_stats.cum)
# 		}
#
# 		filename_stats.update <- filename_update(next_update)
# 		if ((force) || (! file.exists(filename_stats.update)) ) {
# 			if ((next_update == target_update) || save_all) {
# 				stats.update <- calc_diff_tank_stats(stats.cum, stats.update_totals, career = TRUE)   ##
# 				stats.update <- merge_tankopedia(stats.update)              ##
# 				stats.update <- calc_tank_stats(stats.update)               ##
# 				stats.update <- subset(stats.update, battles>0)         ##  since career = TRUE in calc_diff_tank_stats()
# 				print(paste('Update', next_update, ': Saving Update stats'))
...
#
# 			}
# 		} else if (file.exists(filename_stats.update)) {
# 			print(paste('Update', next_update, ': Update stats exists. Skipping...'))
# 		}
#
# 		if ((next_update != target_update) || (save_cum)) {
#
# 			filename_stats.cum.new <- filename_cumulative(next_update);
#
# 			if (file.exists(filename_stats.cum.new) && (! force) ) {
# 				print(paste('Update', next_update, ': cumulative stats exist'))
# 				stats.cum <- qread(filename_stats.cum.new)
# 			} else {
#
# 				print(paste('Update', next_update, ': Generating cumulative stats'))
# 				stats.cum <- rbindlist(list(stats.cum, stats.update_totals), use.names=TRUE, fill=TRUE)
# 				setorder(stats.cum, -last_battle_time)
# 				stats.cum <- stats.cum[, head(.SD,1), by=.(account_id, tank_id)]
# 				stats.cum <- calc_tank_stats_cum(stats.cum)
# 				stats.cum <- merge_tankopedia(stats.cum)
#
# 				if (save_cum && (save_all || (next_update == target_update))) {
# 					print(paste('Update', next_update, ': Saving cumulative stats'))
#
# 					qsave(stats.cum, filename_stats.cum.new, nthreads=n_threads)
# 				}
# 			} # file.exists && ! force
...
#
# 		toc_();  # release
# 		if (next_update == target_update) break
#
# 		curr_update <- next_update
# 		next_update <- get_next_update(curr_update, preliminary)
# 	} # while
