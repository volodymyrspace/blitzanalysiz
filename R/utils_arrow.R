...
ds_calc_tank_stats_diff <- function(DT, cols.diff = NULL) {
  ...
  if (is.null(cols.diff)) {
    cols.diff <- c("all.battles","all.capture_points", "all.damage_dealt", "all.damage_received",
...
ds_calc_tank_stats <- function(DT) {
  ...
  DT[, c('WR.tier.all',
         'avg_dmg.tier.all',
         'spot_rate.tier.all') := list(sum(wins)/first(battles.tier.all),
  ...
  DT[, c('WR.career.tier',
         'avg_dmg.career.tier',
         'spot_rate.career.tier') := list(sum(wins.career)/first(battles.career.tier),
  ...
  DT[battles.career.all > 0,
     c('WR.career.all',
       'avg_dmg.career.all') := list(sum(wins.career) / first(battles.career.all),
...
ds_gen_tank_stats_update <- function(update = get_latest_update(),
  ...
  for (I_split in seq(0, N_split - 1)) {
   ...
   accounts.update <- as.data.table(ds.update_totals  %>%
    ...
    write_dataset(DT.diff, filename.ds.update, format=ds_format,
                  partitioning = c('tier'),
...
ds_load_ts <- function(update = get_latest_update(),
  ...
  if (is.null(modN)) {
    N <- DS.update %>% filter_mode() %>% filter_() %>% nrow()
    modN <- ceiling(N / row.lim)
  }
  if (modN <= 1) {
    modN <- NULL
  } else {
    message(N, ' total rows, taking 1/', modN)
  }
...
ds_load_ts_update <- function(update = get_latest_update(),
                                      ...
                                      server = NULL,
                                      modN = NULL) {
  return(ds_load_ts(update = update,
                            stats_type = "update",
                            cols = cols,
                            tank_tier = tank_tier,
                            tank_type = tank_type,
