...
res <- stats.update[, .(Players = uniqueN(account_id), Battles = sum(all.battles))]
...
Statistics of battles fought during **update `r update``r print_if(preliminary, ' so far')`**. All the stats (WR, damage, battles, etc.) refer to stats during the update, not career stats unless otherwise mentioned. In total, there were `r auto_format(res$Battles)` battles by `r auto_format(res$Players)` players recorded.
