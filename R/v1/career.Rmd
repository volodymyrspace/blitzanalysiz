...
stats.update.perf <- get_stats_tank_perf(stats.update)
...
res <- stats.career[, .(Battles = first(battles.career)),
  by = account_id
]
plot_histogram(res, "Career Battles per Players", update,
  "Battles", "Share of Players %",
  x_lims = c(0, 50e3), x_step = 5000, y_step = 2.5 / 100, x_signif = 2,
...
An average player has played `r as.integer(res$mean)` battles by the end of update `r update`. Median battle count was `r res$median` and the largest number of battles recorded by a single player (in the dataset) was `r auto_format(res$max)` battles. The following tables shows battle counts by N% most inactive players during the update (i.e. "90% most inactive" == "10% most active"). 

### Battles per Player table

{{% expand "Show table" %}}

The table below shows the (max) number of battles played until the end of `r update` by the X% most _inactive_ players. Reading tip: 10% most _active_ players have played more than the 90% most _inactive_ players.

```{r career_battle_count_summary, echo = FALSE}
md_table_quantiles(stats.career[, .(Battles = first(battles.career)),
  by = account_id
...
res <- stats.career[, .(WR = first(WR.career)),
  by = account_id
]
...
h <- hist(stats.career[all.battles > min_battles.career,
  .(WR = first(WR.career)),
  by = account_id
...
len <- length(h$breaks)
res <- data.table("Career WR" = h$breaks[2:len], "Share %" = h$density, "Cumulative share %" = h$cum)[(`Career WR` >= .2) & (`Career WR` < .81)]
md_table(res)
```
{{% /expand %}}
...
res <- stats.career[battles.career > min_battles.career,
  .("Average Tier" = sum(tier * all.battles) / first(battles.career)),
  by = account_id
]
...
res <- stats.career[, .(Battles = sum(all.battles)),
  by = .(account_id, tier)
...
plot_col_discrete_1Y(res, "Most Battles", update, "Tank", "Battles", fill_var = "Tank type", y_pct = FALSE, y_steps = 20)
```
