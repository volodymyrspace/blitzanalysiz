...
for (ndx in order(Nations.str)) {
  ...
  tank_list = get_tank_list(stats = stats.tier, tank_nation = tank_nation, tank_tier = tier)
  if (nrow(tank_list) > 0) { 
    cat(paste('\n##', Nations.str[[ndx]], 'Tanks\n\n'))
    cat(md_tank_list(tank_list, path=".", enough_players = tanks.enough_players), sep = '\n')
  }
}
rm(stats.tier)
```
