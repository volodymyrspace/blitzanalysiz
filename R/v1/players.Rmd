...
page_weight = 1
topN = 20;
battles.all.update <- stats.update[,sum(all.battles)]
var2rm = c('res')
```
...
s <- stats.update[, .(Battles = sum(all.battles)), 
                  by=account_id][,.(mean=mean(Battles), median=median(Battles), max=max(Battles))];
```
...
res <- stats.update[, .(Battles = sum(all.battles)), 
                    by=account_id]
...
md_table_quantiles(stats.update[, .(Battles = sum(all.battles)), 
                           by=account_id][,Battles],'Battles' , breaks = seq(0,1,.05))
```
{{% /expand %}}
...
res <- stats.update[all.battles > min_battles.update, 
                    .(WR = first(WR.global)), 
                    by=account_id]
plot_histogram(res, "Player WR distribution (active players)", update, x_name='WR', y_name='Share of Players' , 
...
```{r update_player_WR}
h <- hist(stats.update[battles.global > min_battles.update, 
                       .(WR = first(WR.global)), 
                       by=account_id][,WR], breaks = 100, plot=FALSE);
h$density <- h$counts/sum(h$counts)
h$cum <- cumsum(h$density);
md_table(data.table("WR"=floor(100*h$mids)/100, 
...
res <- stats.update[battles.global > min_battles.update, 
                    .('Average Tier' = sum(tier*all.battles)/sum(all.battles)), 
                    by=account_id]
...
res <- stats.update[, .('Average Tier' = sum(tier*all.battles)/sum(all.battles)), 
                    by=account_id]
...
res <- stats.update[ DT_filter_enough_battles.tier(stats.update), 
                     .(WR=mean(WR.tier.maxed)), 
                     by=.(tier, account_id)][,.N, by=.(Tier = tier, WR = cut(WR, breaks = buckets.WR, 
                                                                             include.lowest = TRUE))][order(Tier, WR)]
...
res <- stats.update[,.(N = sum(all.battles)), 
                    by=.(Tier=tier, 'Vehicle Class' = type)][order(Tier, `Vehicle Class`)]
...
fill_name = 'Nation'
title <- get_plot_title_share_of_by_by(y_name, x_name, fill_name)
res <- stats.update[,.(N = sum(all.battles)), by=.(Tier=tier, Nation=nation)][order(Tier, Nation)];
...
res <- stats.update[battles.global > min_battles.update, 
                    .('WR' = first(WR.global), 'Region'=first(region)), 
                    by=account_id]
x_breaks = seq(.2,.8,.1)
plot_prob_XY(res, 'WR distribution per Region', update, x_name = 'WR', line_var = 'Region', 
...
res <- stats.update[,.(N = sum(all.battles)), by=.('Region'=region, Nation=nation)][order(`Region`, Nation)]
...
res <- stats.update[,.(N = sum(all.battles)), by=.('Region'=region, 'Vehicle Class'=type)][order(`Region`, `Vehicle Class`)]
