...
cols.sel <- c('account_id', 'tank_id', 'battles', 'battles.career', 'tier',
...
res <- stats.update.perf [ , 
                     ...
                     by=.(tier, account_id)][,.N, by=.(Tier = tier, 
                                                       WR = cut(WR, breaks = buckets.WR, 
...
res <- stats.update[,.(N = sum(battles)), 
                    by=.('Region'=region, Nation=nation)][order(`Region`, Nation)]
