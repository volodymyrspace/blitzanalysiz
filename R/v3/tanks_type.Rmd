...
# Checks
stopifnot(exists("target_page"), exists("stats.tier"), exists("stats.tier.perf"))
page_level_update <- 2
...
tank_type <- get_path_elem(target_page, 4)
tank_type <- path2tank_type(tank_type)
min_battles.tier <- get_min_battles_tier(tier)
min_battles.tank <- floor(min_battles.tier / 2)
min_battles.grind <- get_grind_len(tier)
page_weight <- match(tank_type, Tank_Types)

stats.tier.type <- subset(stats.tier, type == tank_type)
stats.tier.type.perf <- subset(stats.tier.perf, type == tank_type)

var2rm <- c("res", "stats.tier.type", "stats.tier.type.perf")
tanks.enough_players <- get_tanks_enough_players(stats.tier.type.perf)
```
---
title: "Tier `r get_tier_roman(tier)` `r tank_type`s"
menutitle: `r tank_type`s
date: `r today("UTC")`
author: Jylpah@gmail.com
output: md_document
weight: `r page_weight`
description: "Stats of tier `r paste(tier.roman, tank_type)`s in World of Tanks Blitz. Update `r update`"
tags: [ "`r update`", "Tier `r tier.roman`", "Tanks/Tier", "`r tank_type`" ]
disableToc: false
---
...
tank_list <- get_tank_list(stats = stats.tier.type, tank_tier = tierN, tank_type = tank_type)
...
The graph shows tier [`r tier.roman`](..) `r tank_type`s by _Relative WR_. That is the average of players' WR 
in a tank compared to their average WR at the tier (in all same tier tanks). Relative WR is a measure of the performance/strength of the tank.
...
```{r fig_tanks_tier_type_Relative_WR}
res <- stats.tier.type.perf[DT_filter_enough_battles.rWR(stats.tier.type.perf),
    .(
        "Average WR" = mean(WR),
...
{{% expand "Show table" %}}
```{r tanks_tier_type_Relative_WR, echo = FALSE}
...
```{r fig_tanks_tier_type_Relative_WR_topN_65_}
res <- stats.tier.type.perf[
    DT_filter_enough_battles.rWR(stats.tier.type.perf) &
        (WR.tier.maxed > .65),
    .(
        "Average WR" = mean(WR),
        "Relative WR" = mean(rWR),
...
```
```{r tanks_tier_type_Relative_WRN_65_, results='asis'}
if (nrow(res) == 0) {
...
```{r fig_tanks_tier_type_Relative_WR_topN_55_65}
res <- stats.tier.type.perf[
    DT_filter_enough_battles.rWR(stats.tier.type.perf) &
...
```
```{r tanks_tier_type_Relative_WRN_55_65, results='asis'}
if (nrow(res) == 0) {
    cat("Not enough players to calculate Relative WR\n")
}
```
...
```{r fig_tanks_tier_type_Relative_WR_topN_45_55}
res <- stats.tier.type.perf[
    DT_filter_enough_battles.rWR(stats.tier.type.perf) &
...
if (nrow(res) > 0) {
    ...
    plot_col_discrete_1Y(res, title, update, x_name, y_name, fill_var = "Tank type", y_pct = TRUE, top = TRUE, tier = tier)
}
```
```{r tanks_tier_type_Relative_WRN_45_55, results='asis'}
if (nrow(res) == 0) {
    cat("Not enough players to calculate Relative WR\n")
}
```
...
```{r fig_tanks_tier_type_Relative_WR_topN_0_45}
res <- stats.tier.type.perf[
    DT_filter_enough_battles.rWR(stats.tier.type.perf) &
        (WR.tier.maxed <= .45),
    .(
...
```
```{r tanks_tier_type_Relative_WRN_0_45, results='asis'}
if (nrow(res) == 0) {
    cat("Not enough players to calculate Relative WR\n")
}
```
...
{{% expand "Show table" %}}
```{r tanks_tier_type_WR, echo = FALSE}
md_table(res, update_path_lvl = page_level_update, enough_players = tanks.enough_players)
```
{{% /expand %}}
...
This graph shows tier [`r tier.roman`](..) `r tank_type`s ranked by players' average damage in the tanks.

```{r fig_tanks_tier_type_avg_dmg}
res <- stats.tier.type.perf[, .(
...
plot_col_discrete_1Y(res, title, update, x_name, y_name,
    fill_var = "Tank type",
    y_pct = FALSE, top = TRUE, tier = tier, y_steps = 10
)
```
...
{{% expand "Show table" %}}
```{r tanks_tier_type_avg_dmg, echo = FALSE}
md_table(res, update_path_lvl = page_level_update, enough_players = tanks.enough_players)
```
{{% /expand %}}
...
This graph shows tier [`r tier.roman`](..) `r tank_type`s ranked by players' average kills per battle.

```{r fig_tanks_tier_type_avg_kills}
res <- stats.tier.type.perf[, .(
...
{{% expand "Show table" %}}
```{r tanks_tier_type_avg_kills, echo = FALSE}
md_table(res, update_path_lvl = page_level_update, enough_players = tanks.enough_players)
```
{{% /expand %}}
...
This graph shows tier [`r tier.roman`](..) `r tank_type`s ranked by players' spot rate in the tanks.

```{r fig_tanks_tier_type_spot_rate}
res <- stats.tier.type.perf[, .(
...
{{% expand "Show table" %}}
```{r tanks_tier_type_spot_rate, echo = FALSE}
md_table(res, update_path_lvl = page_level_update, enough_players = tanks.enough_players)
```
{{% /expand %}}
...
This graph shows tier [`r tier.roman`](..) `r tank_type`s ranked by players' hit rate in the tanks.

```{r fig_tanks_tier_type_hit_rate}
res <- stats.tier.type.perf[, .(
...
{{% expand "Show table" %}}
```{r tanks_tier_type_hit_rate, echo = FALSE}
md_table(res, update_path_lvl = page_level_update, enough_players = tanks.enough_players)
```
{{% /expand %}}
...
```{r fig_tanks_tier_type_most_popular}
res <- stats.tier.type[, .(
    "Players" = uniqueN(account_id),
    "Battles" = sum(battles),
    "Battles/Player" = mean(battles),
...
res <- prep_plot_tank(res, y_name)
title <- get_plot_title_top_tank_type(y_name, tank_type, top = top, topN = NULL)
plot_col_discrete_1Y(res, title, update, x_name, y_name, fill_var = "Tank type", y_pct = FALSE, top = top, tier = tier, y_steps = 10)
```
...
```{r fig_tanks_tier_type_most_played}
res <- stats.tier.type[, .(
    "Players" = uniqueN(account_id),
    "Battles" = sum(battles),
    "Battles/Player" = mean(battles),
...
{{% expand "Show table" %}}
```{r tanks_tier_type_most_played, echo = FALSE}
md_table(res, update_path_lvl = page_level_update, enough_players = tanks.enough_players)
```
{{% /expand %}}
