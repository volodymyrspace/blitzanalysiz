...
stopifnot(exists("target_page"), exists("stats.tier.perf"))
...
stats.tank <- subset(stats.tier.type.perf, tank_id == tank_ID)

if (!has_enough_players(stats.tank, tank_ID)) {
    knit_exit(DELETE_PAGE)
}

var2rm <- c("res", "stats.tank", "stats.tier.type.perf")
if (nrow(stats.tank) > 2000) {
...
output: md_document
description: "`r tank_name` tank stats. A tier `r paste(tier.roman, tank_type)` in World of Tanks Blitz. Update `r update`"
...
res <- stats.tier.type.perf[DT_filter_enough_battles(stats.tier.type.perf),
    .(
        "WR at Tier" = WR.tier.maxed,
        #' WR (Tank)' = WR,
...
plot_quantile_Y(res, title, update, by_measure, perf_measure,
    fill_var = "Tank", fill_elems = tank_name,
    bin_breaks = bin_breaks, x_pct = TRUE, y_pct = FALSE, y_steps = 10,
    palette.fill = palette.vehicle_class[[get_tank_type(tank_id)]],
    palette.annotate = palette.vehicle_class.lines[[get_tank_type(tank_id)]],
...
plot_quantile_Y(res, title, update, by_measure, perf_measure,
    ...
    palette.fill = palette.vehicle_class[[get_tank_type(tank_id)]],
    palette.annotate = palette.vehicle_class.lines[[get_tank_type(tank_id)]],
    avg_name = title.tier.tank_type, tier = tier
)
```
...
res <- stats.tank[
    ,
    .(
        "Average Battle Lifetime" = battle_life_time / battles,
        "Spot Rate" = spot_rate,
        "WR" = WR,
        "Battles" = battles
    )
][`Average Battle Lifetime` <= 7 * 60]
